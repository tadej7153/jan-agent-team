...
const ChatInput = memo(function ChatInput({
  ...
  const setThreadAgentBinding = useAgentTeams((state) => state.setThreadBinding)
  // When projectId is present, treat as normal chat (disable agent mode UI)
  const isAgentMode = threadAgentBinding.enabled
      ...
      if (isTemporaryChat) {
        ...
        if (isAgentMode && agentModeKey !== TEMPORARY_CHAT_ID) {
          ...
          if (threadAgentBinding.enabled) {
            useAgentTeams
              .getState()
              .setThreadBinding(TEMPORARY_CHAT_ID, threadAgentBinding)
        ...
        if (isAgentMode) {
          ...
          useAgentMode.getState().removeThread(agentModeKey)
          if (threadAgentBinding.enabled) {
            useAgentTeams.getState().setThreadBinding(newThread.id, threadAgentBinding)
            useAgentTeams.getState().removeThreadBinding(agentModeKey)
          }
        }
