...
export const MessageItem = memo(
  ({
    ...
    const agentAvatar =
      typeof metadata?.agentAvatar === 'string' ? metadata.agentAvatar : undefined
    const agentRole =
      typeof metadata?.agentRole === 'string' ? metadata.agentRole : undefined
      ...
      >
        {message.role === 'assistant' && agentName && (
          <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
            <div className="size-6 flex items-center justify-center rounded-md bg-secondary text-sm">
              {agentAvatar || <IconRobot size={14} />}
            </div>
            <span className="font-medium text-foreground">{agentName}</span>
            {agentRole && <span className="truncate">{agentRole}</span>}
          </div>
        )}
